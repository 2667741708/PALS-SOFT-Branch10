...
# ==============================================================================
# Branch 3: 针对众包高噪数据的候选标签平滑化 (PALS)
# Core: Adaptive label smoothing within candidate sets based on KNN-model agreement
# ==============================================================================
...
# --- 修改部分开始 ---
    ...
    # [Branch3] Candidate-aware label smoothing
    parser.add_argument('--branch3_base_smooth_rate', type=float, default=0.1,
                        help='[Branch3] Base label smoothing rate within candidate set')
    parser.add_argument('--branch3_max_smooth_rate', type=float, default=0.5,
                        help='[Branch3] Max smoothing rate for high-disagreement samples')
    parser.add_argument('--branch3_agreement_power', type=float, default=2.0,
                        help='[Branch3] Power for agreement-based smoothing schedule')
    parser.add_argument('--branch3_mixup_alpha', type=float, default=0.5,
                        help='[Branch3] MixUp alpha for consistency regularization')
    parser.add_argument('--branch3_consistency_rampup', type=int, default=50,
                        help='[Branch3] Epochs for consistency regularization rampup')
...
def _branch3_candidate_aware_smoothing(fused_dist, cand_mask, crowd_prior, 
                                        model_preds, knn_raw, args, eps=1e-8):
    """Apply adaptive label smoothing within candidate label sets.
    
    For each sample:
    1. Compute agreement between model and KNN predictions
    2. High agreement -> low smoothing (keep sharp distribution)  
    3. Low agreement -> high smoothing (spread probability among candidates)
    
    Args:
        fused_dist: [N, C] fused probability distribution after candidate projection
        cand_mask: [N, C] candidate set mask (binary for PLL, soft for crowdsourced)
        crowd_prior: [N, C] or None, crowdsource voting prior
        model_preds: [N, C] model predictions
        knn_raw: [N, C] KNN aggregated predictions
        args: argument namespace
    Returns:
        smoothed_dist: [N, C] smoothed distribution
    """
    N, C = fused_dist.shape
    base_rate = float(getattr(args, 'branch3_base_smooth_rate', 0.1))
    max_rate = float(getattr(args, 'branch3_max_smooth_rate', 0.5))
    power = float(getattr(args, 'branch3_agreement_power', 2.0))
    
    # Per-sample agreement: cosine similarity between model and KNN predictions
    model_norm = F.normalize(model_preds, p=1, dim=1)
    knn_norm = F.normalize(knn_raw, p=1, dim=1)
    agreement = (model_norm * knn_norm).sum(dim=1)  # [N], range [0, 1]
    
    # Adaptive smoothing rate: high agreement -> low rate, low agreement -> high rate
    smooth_rate = base_rate + (max_rate - base_rate) * (1.0 - agreement.pow(power))  # [N]
    smooth_rate = smooth_rate.unsqueeze(1)  # [N, 1]
    
    # Determine the support set for smoothing
    if crowd_prior is not None:
        # For crowdsourced: use crowd_prior as the uniform-within-candidates distribution
        cand_uniform = crowd_prior / (crowd_prior.sum(dim=1, keepdim=True) + eps)
    else:
        # For PLL: uniform distribution over candidate classes
        cand_count = cand_mask.sum(dim=1, keepdim=True).clamp(min=1)
        cand_uniform = cand_mask / cand_count
    
    # Apply smoothing: (1 - r) * fused + r * uniform_within_candidates
    smoothed = (1.0 - smooth_rate) * fused_dist + smooth_rate * cand_uniform
    
    # Re-normalize
    smoothed = smoothed / (smoothed.sum(dim=1, keepdim=True) + eps)
    
    return smoothed, agreement
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # ============================================================
    # [Branch3] Apply candidate-aware label smoothing before 2nd propagation
    # ============================================================
    propagation_input_2_raw = base_dist / (base_dist.sum(dim=1, keepdim=True) + eps)
    
    if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
        cand_for_smooth = crowd_prior if crowd_prior is not None else static_cand_mask.float()
        propagation_input_2, b3_agreement = _branch3_candidate_aware_smoothing(
            propagation_input_2_raw, cand_for_smooth, crowd_prior,
            model_preds, knn1_raw, args, eps)
        logger.info(f"   [Branch3] Label smoothing applied: mean_agreement={b3_agreement.mean():.4f}, "
                     f"base_rate={getattr(args, 'branch3_base_smooth_rate', 0.1)}, "
                     f"max_rate={getattr(args, 'branch3_max_smooth_rate', 0.5)}")
    else:
        propagation_input_2 = propagation_input_2_raw
