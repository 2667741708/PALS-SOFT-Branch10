...
python "三方共识更新_exp10_knn1_softprop_topo_daes__节点可靠性_余弦降低的lsr_v1.py" \
    --dataset CUB200 \
    --train_root ./data \
    --lpi 10 \
    --pr 0.05 \
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name 三方共识更新_exp10_knn1_softprop_topo_daes__节点可靠性_余弦降低的lsr_v1/CUB200pr0.05nr0.2e250exp_topdaes_hl15_del0.5lsr0.5_0.0_123 \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
...
python "三方共识更新_exp10_knn1_softprop_topo_daes__节点可靠性_余弦降低的lsr_v1.py" \
    ...
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name 三方共识更新_exp10_knn1_softprop_topo_daes__节点可靠性_余弦降低的lsr_v1/CUB200pr0.05nr0.2e250topology_daes_topdaes_hl15_del0.5lsr0.5_0.0_123 \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
    --consistency_weight 1.0 \
...
python "节点可靠性_降低lsr_v2.py" \
--dataset CIFAR100 \
--out topology_daes \
--exp_name "节点可靠性_降低lsr_v2/pr0.05nr0.5e100exp_top_hl15_geofuse+softprop12lsr0.0_0.0_dbt0.01" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
--pr 0.05 --nr 0.5 --epochs 100 --sim_mode_1 exp --sim_mode_2 topology_daes \
--lr 0.1 --wd 1e-3 --history_len 15 --k_val 15 --lsr 0.0 \
--enable_knn1_soft_prop --knn1_soft_prop_max_w 0.5 --enable_knn1_model_fuse --seeds 1 2 --daes_base_tau 0.01

python "三方共识更新_exp10_knn1_softprop_topo_daes__节点可靠性_余弦降低的lsr_v1.py" \
--dataset CIFAR100H \
--out topology_daes \
--exp_name "三方共识更新_exp10_knn1_softprop_topo_daes__节点可靠性_余弦降低的lsr_v1/100Hpr0.5nr0.2e100exp_top_hl15_geofuse+softprop23lsr0.5_0.0" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
--pr 0.5 --nr 0.2 --epochs 100 --sim_mode_1 exp --sim_mode_2 topology \
--lr 0.1 --wd 1e-3 --history_len 15 --k_val 15 --lsr 0.0 \
...
--dataset CIFAR10 \
--out topology_daes \
--exp_name "节点可靠性_降低lsr_v2/ cifar10pr0.5nr0.2e500exp_topdaes_geofuse+gardprop_lsr0.5_0.0_dbt0.01" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
--pr 0.5 --nr 0.2 --epochs 500 --sim_mode_1 exp --sim_mode_2 topology_daes \
--lr 0.1 --wd 1e-3 --history_len 15 --k_val 15 --lsr 0.0 \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    for i in range(iterations):
        if i == 0:
            ...
            # --- 步骤 A: 选择输入源 ---
            # --- 步骤 A: 选择输入源 (方案B: 算术加权和融合 + 提前饱和调度) ---
            if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
                eps = float(getattr(args, 'knn1_soft_prop_eps', 1e-8))
                max_w = float(getattr(args, 'knn1_soft_prop_max_w', 0.5))
                
                # 1. 动态权重调度：在 50% 的 epochs 时提前到达 max_w (通常为 1.0)
                half_epochs = float(max(getattr(args, 'epochs', 2) / 2.0, 1.0))
                w_model = float(np.clip(max_w * (epoch / half_epochs), 0.0, max_w))

                # 2. 计算邻域的模型几何共识 (Geo)
                neighbor_idx = neighbors_mh[:, 1:]
                w_norm = raw_sim[:, 1:] / (raw_sim[:, 1:].sum(dim=1, keepdim=True) + eps)
                
                # 如果你希望模型预测更加锐利，可以在此处将 model_preds 替换为硬化版本 (F.one_hot)
                # 但在算术融合下，保持软分布通常能提供更好的梯度平滑性
                geo = torch.einsum('nk,nkc->nc', w_norm, model_preds[neighbor_idx])
                geo = geo / (geo.sum(dim=1, keepdim=True) + eps)

                # 3. 核心修改：线性加权和融合 (Arithmetic Mean)
                # 逻辑"或"特性：只要一方有概率，融合后就不会为 0，彻底解决数值锁死问题
                fused_dist = (1.0 - w_model) * knn1_raw + w_model * geo
                
                # 4. 再次归一化以确保作为合法概率分布进入下一轮拓扑传播
                base_dist = fused_dist / (fused_dist.sum(dim=1, keepdim=True) + eps)
                
                # (可选日志) 记录当前融合权重，便于在控制台观察接管进度
                if i == 1 and epoch % 10 == 0 and hasattr(logger, 'debug'):
                    logger.debug(f" [Fusion] Epoch {epoch}: w_model reached {w_model:.3f}")
            else:
                base_dist = knn1_raw
...
def run_single_experiment(args):
    ...
    for epoch in range(total_epochs):
        epoch_start_time = time.time()
        start_lsr, end_lsr = 0.00, 0.0
        fraction = epoch / max(total_epochs/2 - 1, 1)
