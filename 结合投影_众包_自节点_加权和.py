...
python 结合投影_众包_自节点_加权和.py \
    ...
    --lpi 10 \
    --out ./topology_daes \
    --exp_name 结合投影_众包_自节点_加权和/Treeversity/LPI10/del1.0_lsr0.0k5_单头_可投到众包_hl50_weighted_sum \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
    ...
    --lr_decay_rate 0.2 \
    --seeds 1 2 3 \
    --detailed_log    --cuda_dev 0 --k_val 5 --knn_heads 1 --fusion_mode weighted_sum
python 结合投影_众包_自节点_加权和.py \
    --train_root ./data \
    --dataset Benthic \
    --lpi 10 \
    --out ./topology_daes \
    --exp_name 结合投影_众包_自节点_加权和/Benthic/LPI10/del1.0_lsr0.0k5_单头_可投到众包_hl50_weighted_sum_二次迭代无投影 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
    ...
    --lr_decay_rate 0.2 \
    --seeds 1 2 3 \
    --detailed_log    --cuda_dev 0 --k_val 5 --history_len 50 --knn_heads 1 --fusion_mode weighted_sum --enable_knn1_model_fuse 
...
python "结合投影_众包_自节点_加权和.py" \
    ...
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name 结合投影_众包_自节点_加权和/CUB200/pr0.05nr0.2e250topology_daes_topdaes_hl15_del0.5lsr0.0_123_step_1head_weighted_sum \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
    ...
    --num_workers 4 \
    --k_val 15 \
    --history_len 15 --fusion_mode weighted_sum --enable_knn1_model_fuse --knn_heads 1 \
...
--dataset CIFAR100 \
--out topology_daes \
--exp_name "结合投影_众包_自节点_加权和/CIFAR100/pr0.05nr0.5e500top_top_hl15_weighted_sum" \
--pr 0.05 --nr 0.5 --epochs 500 --sim_mode_1 topology_daes --sim_mode_2 topology_daes \
--lr 0.1 --wd 1e-3 --history_len 15 --k_val 15 --lsr 0.0 --fusion_mode weighted_sum --enable_knn1_model_fuse \
...
#     """
#     博士级增强版：双源感知可靠集筛选 (支持众包先验软投影与 PLL 硬掩码分离)
#     执行三次迭代：第1次(投影前) -> 投影操作 -> 第2次(投影后) -> 第3次(新增:投影后的额外传播)
#     """
#     N = features.shape[0]
#     dataset = trainloader.dataset
#     eps_stable = 1e-8
...
#         overall_pred_prior = propagation_input_2.argmax(dim=1)
#         overall_acc_prior = (overall_pred_prior == clean_labels).float().mean().item()
#         logger.info(f"📊 [Stage 1.8] After Prior Fusion (Projection)")
#         logger.info(f"   ├─ Selected: {int(size_prior)} samples | Acc: {acc_prior*100:.2f}% | Overall Acc: {overall_acc_prior*100:.2f}%")

#     # ==============================================================================
#     # 第二次迭代 (i = 1): 投影后的第 1 次 KNN 传播
#     # ==============================================================================
#     refined_sim_2 = get_weight_matrix(args.sim_mode_2, raw_sim, neighbors_mh, propagation_input_2, args)
...
#     neighbor_vals_2 = propagation_input_2[neighbors_mh]
#     weighted_votes_2 = torch.einsum('nk,nkc->nc', voting_weights_2, neighbor_vals_2)
#     curr_soft_out_stage2 = F.softmax(weighted_votes_2, dim=1)

#     with torch.no_grad():
#         mask_st2, pred_st2, acc_st2, size_st2 = _filter_logic(curr_soft_out_stage2)
#         overall_pred_st2 = curr_soft_out_stage2.argmax(dim=1)
#         overall_acc_st2 = (overall_pred_st2 == clean_labels).float().mean().item()
#         logger.info(f"📊 [Stage 2] After 2nd Propagation")
#         logger.info(f"   ├─ Selected: {int(size_st2)} samples | Acc: {acc_st2*100:.2f}% | Overall Acc: {overall_acc_st2*100:.2f}%")

#     # ==============================================================================
#     # ⭐ 新增：第三次迭代 (i = 2): 投影后的第 2 次 KNN 传播
#     # ==============================================================================
#     propagation_input_3 = curr_soft_out_stage2

#     # 复用 sim_mode_2 构建第三次传播的拓扑矩阵
#     refined_sim_3 = get_weight_matrix(args.sim_mode_2, raw_sim, neighbors_mh, propagation_input_3, args)
    
#     voting_weights_3 = refined_sim_3
#     neighbor_vals_3 = propagation_input_3[neighbors_mh]
#     weighted_votes_3 = torch.einsum('nk,nkc->nc', voting_weights_3, neighbor_vals_3)
#     curr_soft_out = F.softmax(weighted_votes_3, dim=1)

#     # ==============================================================================
#     # 最终的可靠集筛选和状态更新 (基于第三次传播结果)
#     # ==============================================================================
#     mask_final, pred_final, acc_rel_final, size_rel_final = _filter_logic(curr_soft_out)
#     mask_final = mask_final.float()
...
#     logger.info(f"📊 [Stage 3] Final Selection (After 3rd Propagation)")
#     logger.info(f"   ├─ Selected: {int(size_rel_final)} samples | Acc: {acc_rel_final*100:.2f}% | Overall Acc: {overall_acc_final*100:.2f}%")
#     logger.info(f"{'='*80}\n")
