...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    refined_sim_1, label_imp_1 = get_weight_matrix(args.sim_mode_1, raw_sim, neighbors_mh, propagation_input_1, args)
    
    voting_weights_1 = refined_sim_1
    neighbor_vals_1 = propagation_input_1[neighbors_mh]
    neighbor_imp_1 = label_imp_1[neighbors_mh]
    weighted_votes_1 = torch.einsum('nk,nk,nkc->nc', voting_weights_1, neighbor_imp_1, neighbor_vals_1)
    ...
    refined_sim_2, label_imp_2 = get_weight_matrix(args.sim_mode_2, raw_sim, neighbors_mh, propagation_input_2, args)
    
    voting_weights_2 = refined_sim_2
    neighbor_vals_2 = propagation_input_2[neighbors_mh]
    neighbor_imp_2 = label_imp_2[neighbors_mh]
    weighted_votes_2 = torch.einsum('nk,nk,nkc->nc', voting_weights_2, neighbor_imp_2, neighbor_vals_2)
...
def get_weight_matrix(mode, raw_D, neighbors_indices, ref_soft_labels, args):
    ...
    """
    label_importance = torch.ones(raw_D.shape[0], device=raw_D.device)
    if mode == 'daes':
        refined_D = get_adaptive_affinity_matrix(raw_D, neighbors_indices, ref_soft_labels, args)
    elif mode == 'topology':
        refined_D, label_importance = get_topology_guided_affinity(
        ...
        refined_D, label_importance = get_topology_daes_affinity(raw_D, neighbors_indices, ref_soft_labels, args)
    elif mode == 'exp':
        refined_D = torch.exp(raw_D / 0.1)
    else: # 'linear' or 'none'
        refined_D = raw_D
        
    return refined_D, label_importance
