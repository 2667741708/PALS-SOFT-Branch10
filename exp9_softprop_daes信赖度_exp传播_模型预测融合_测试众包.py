...
python exp9_softprop_daes信赖度_exp传播_模型预测融合_测试众包.py \
    ...
    --lpi 3 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播_模型预测融合_测试众包/Treeversity/lr0.01LPI3_Run_1del1_top_top_lsr0.0_k_15_new2 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
python exp9_softprop_daes信赖度_exp传播_模型预测融合_测试众包.py \
    ...
    --lpi 3 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播_模型预测融合_测试众包/Treeversity/lr0.01LPI3_Run_1del1_top_top_lsr0.0_k_15_new3 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
# CUB200 示例（不开启模型融合）
python "三方共识更新_exp9_softprop精简版_self_node.py" \
    --dataset CUB200 \
    --train_root ./data \
    --lpi 10 \
    --pr 0.05 \
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name 三方共识更新_exp9_softprop精简版_self_node/lr_0.1CUB200pr0.05nr0.2e250top_top_hl15_exp9_fuse_softpropdel0.25lsr0.5_123 \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # 4. 投影到当前的候选标签集 (0/1 掩码 或 众包加权软分布)
    cand_prior = static_cand_mask.float()
    projected_dist = geo_mean_norm * cand_prior
    
    # 5. 第二次归一化，生成下一阶段的输入和可靠度评估基础
    phase2_input = projected_dist / (projected_dist.sum(dim=1, keepdim=True) + 1e-12)

    # 计算节点自身的可信赖度 (使用融合投影后，落在自节点初始预测上的概率质量)
    self_preds = curr_soft_out.argmax(dim=1)
    reliability_scores = phase2_input.gather(1, self_preds.unsqueeze(1)).squeeze(1)
    # ==============================================================================
