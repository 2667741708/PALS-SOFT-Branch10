...
# CUB200 示例（不开启模型融合）
python "双自daes信赖度_exp传播_模型预测融合_显式区分众包普通.py" \
    --dataset CUB200 \
    --train_root ./data \
    --lpi 10 \
    --pr 0.05 \
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name 双自daes信赖度_exp传播_模型预测融合_显式区分众包普通/lr_0.1CUB200pr0.05nr0.2e250top_top_hl15_del0.5lsr0.5_123_k15cosine \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # ==============================================================================
    raw_sim_neighbors = raw_sim[:, :]
    neighbors_indices_only = neighbors_mh[:, :]
