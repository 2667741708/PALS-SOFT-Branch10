...
"""
# ==============================================================================
# Branch 1: 基于信息熵的样本级动态掩码与置信度自适应加权
# Core: Replace global w_model with per-sample w_i = exp(-gamma * H(p_i))
# ==============================================================================
...
# --- 修改部分开始 ---
    ...
    # [Branch1] Entropy-based sample-level dynamic fusion
    parser.add_argument('--branch1_entropy_gamma', type=float, default=5.0,
                        help='[Branch1] Gamma for exponential entropy decay: w_i = exp(-gamma * H(p_i))')
    parser.add_argument('--branch1_ema_threshold', action='store_true',
                        help='[Branch1] Use EMA dynamic threshold for entropy masking')
    parser.add_argument('--branch1_ema_momentum', type=float, default=0.9,
                        help='[Branch1] EMA momentum for threshold update')
    parser.add_argument('--branch1_mask_quantile', type=float, default=0.3,
                        help='[Branch1] Quantile for entropy masking (samples above this quantile get w=0)')
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
        ...
        # ============================================================
        # [Branch1] Sample-level entropy-based adaptive fusion weight
        # ============================================================
        # Compute per-sample Shannon entropy of model predictions
        model_entropy = -torch.sum(model_preds * torch.log(model_preds + eps), dim=1)  # [N]
        max_entropy = np.log(args.num_classes)
        norm_entropy = model_entropy / (max_entropy + eps)  # normalized to [0, 1]
        
        # Exponential decay: confident samples (low entropy) get high weight
        gamma_b1 = float(getattr(args, 'branch1_entropy_gamma', 5.0))
        w_per_sample = torch.exp(-gamma_b1 * norm_entropy)  # [N], range (0, 1]
        
        # Optional: EMA-based dynamic masking
        if getattr(args, 'branch1_ema_threshold', False):
            mask_quantile = float(getattr(args, 'branch1_mask_quantile', 0.3))
            entropy_threshold = torch.quantile(norm_entropy, 1.0 - mask_quantile)
            # Zero out weight for high-entropy samples
            w_per_sample = w_per_sample * (norm_entropy <= entropy_threshold).float()
        
        # Clamp to max_w
        max_w_val = float(getattr(args, 'knn1_soft_prop_max_w', 1.0))
        w_per_sample = torch.clamp(w_per_sample, 0.0, max_w_val)
        
        # Log statistics
        logger.info(f"   [Branch1] Entropy fusion: mean_w={w_per_sample.mean():.4f}, "
                     f"std_w={w_per_sample.std():.4f}, "
                     f"mean_entropy={norm_entropy.mean():.4f}")
        
        fusion_mode = getattr(args, 'fusion_mode', 'weighted_sum')
        w_sample_expanded = w_per_sample.unsqueeze(1)  # [N, 1] for broadcasting
        
        if fusion_mode == 'geometric':
            log_knn = torch.log(knn1_raw + eps)
            log_geo = torch.log(geo + eps)
            # Per-sample weighted geometric fusion in log space
            log_fused_temp = log_knn + w_sample_expanded * log_geo
            soft_model_fused = F.softmax(log_fused_temp, dim=1)
        else:  # weighted_sum
            # Per-sample weighted sum fusion
        ...
        with torch.no_grad():
            ...
            logger.info(f"\U0001f4ca [Stage 1.5] After Model Fusion ({fusion_mode}, Branch1-Entropy)")
