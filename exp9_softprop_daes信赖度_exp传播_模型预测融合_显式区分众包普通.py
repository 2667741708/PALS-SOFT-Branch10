"""
python exp9_softprop_daes信赖度_exp传播_模型预测融合_测试众包.py \
    --dataset Treeversity \
    --lpi 3 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播_模型预测融合_测试众包/Treeversity/lr0.01LPI3_Run_1del1_top_top_lsr0.0_k_15_new \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
# CUB200 示例（不开启模型融合）
python "exp9_softprop_daes信赖度_exp传播_模型预测融合_显式区分众包普通.py" \
    --dataset CUB200 \
    --train_root ./data \
    --lpi 10 \
    --pr 0.05 \
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播_模型预测融合_显式区分众包普通/lr_0.1CUB200pr0.05nr0.2e250top_top_hl15_del0.5lsr0.5_123_k15cosine \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
...
class TemporalStateManager:
    ...
    def get_salvage_mask(self):
        ...
        # 修改后 (例如允许 15 轮内进过 1-2 次可靠集，或者只看最近几轮)
        always_unreliable = (rel_stack[-5:].sum(dim=0) == 0)
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # ==============================================================================
    # 方案 1：显式区分逻辑 (Manual If-Else Switch)
    # ==============================================================================

    # A. 基础计算：投影到候选集并生成 Phase 2 输入
    ...
    # B. 核心判断
    # 众包数据集：Treeversity, Benthic, Plankton
    # 普通数据集：CIFAR10, CIFAR100, CIFAR100H, CUB200
    is_crowd_data = any(name in args.dataset for name in ['Treeversity', 'Benthic', 'Plankton'])

    if is_crowd_data:
        # 逻辑 A：锚点一致性检查 (对众包的高投票先验极度友好)
        # 提取动态修正后的最强项
        dynamic_anchor = curr_soft_out.argmax(dim=1, keepdim=True)
        # 计算融合意见在锚点上的集中度
        reliability_scores = phase2_input.gather(1, dynamic_anchor).squeeze(1)
    else:
        # 逻辑 B：合法空间概率总和 (对普通 PL 的平权候选集更具容错性)
        # 只要融合意见落在合法候选圈内，即视为可信
...
def run_single_experiment(args):
    ...
    for epoch in range(total_epochs):
        ...
        state_manager.update_history(
            ...
            pruned_pl=knn_pl, 
            geo_pl=model_pl, # 假设 model_pl 是概率
