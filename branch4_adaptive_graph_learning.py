...
# ==============================================================================
# Branch 4: 非参数化自适应图神经学习
# Core: Sample-wise adaptive K + candidate-aware Jaccard edge pruning
# ==============================================================================
...
# --- 修改部分开始 ---
    ...
    # [Branch4] Adaptive graph learning
    parser.add_argument('--branch4_k_min', type=int, default=5,
                        help='[Branch4] Minimum K for adaptive KNN')
    parser.add_argument('--branch4_k_max', type=int, default=30,
                        help='[Branch4] Maximum K for adaptive KNN')
    parser.add_argument('--branch4_density_percentile', type=float, default=0.5,
                        help='[Branch4] Percentile for local density estimation')
    parser.add_argument('--branch4_jaccard_threshold', type=float, default=0.0,
                        help='[Branch4] Min Jaccard overlap to keep an edge (0=keep all)')
    parser.add_argument('--branch4_dual_graph_weight', type=float, default=0.3,
                        help='[Branch4] Weight for label-correlation graph vs instance-similarity graph')
...
def _branch4_compute_adaptive_k(raw_sim_full, k_min, k_max, density_percentile=0.5):
    """Compute per-sample adaptive K based on local density.
    Samples in dense regions use smaller K; samples in sparse regions use larger K.
    
    Args:
        raw_sim_full: [N, K_max+1] similarity matrix from KNN search with K_max
        k_min: minimum K
        k_max: maximum K (should match the KNN search K)
        density_percentile: percentile threshold for density
    Returns:
        adaptive_k: [N] tensor of per-sample K values
    """
    N = raw_sim_full.shape[0]
    # Local density: mean similarity to k_min nearest neighbors
    local_density = raw_sim_full[:, 1:k_min+1].mean(dim=1)  # exclude self (index 0)
    
    # Normalize density to [0, 1]
    d_min = local_density.min()
    d_max = local_density.max()
    if d_max - d_min < 1e-8:
        norm_density = torch.ones_like(local_density) * 0.5
    else:
        norm_density = (local_density - d_min) / (d_max - d_min)
    
    # Dense samples -> small K, sparse samples -> large K
    # adaptive_k = k_max - (k_max - k_min) * density
    adaptive_k = (k_max - (k_max - k_min) * norm_density).long()
    adaptive_k = adaptive_k.clamp(k_min, k_max)
    
    return adaptive_k

def _branch4_jaccard_edge_pruning(neighbor_idx, cand_mask, jaccard_threshold=0.0, eps=1e-8):
    """Prune KNN edges based on Jaccard overlap of candidate label sets.
    
    Args:
        neighbor_idx: [N, K+1] neighbor indices
        cand_mask: [N, C] binary/soft candidate mask
        jaccard_threshold: minimum Jaccard overlap to keep edge
    Returns:
        edge_weights: [N, K+1] multiplicative weights (0 for pruned edges)
    """
    N, K_plus_1 = neighbor_idx.shape
    
    if jaccard_threshold <= 0.0:
        return torch.ones(N, K_plus_1, device=neighbor_idx.device)
    
    # Binarize candidate mask for Jaccard computation
    cand_binary = (cand_mask > 0).float()  # [N, C]
    
    # Get candidate sets for center nodes and their neighbors
    center_cand = cand_binary.unsqueeze(1).expand(-1, K_plus_1, -1)  # [N, K+1, C]
    neighbor_cand = cand_binary[neighbor_idx]  # [N, K+1, C]
    
    # Jaccard = |intersection| / |union|
    intersection = (center_cand * neighbor_cand).sum(dim=2)  # [N, K+1]
    union = ((center_cand + neighbor_cand) > 0).float().sum(dim=2)  # [N, K+1]
    jaccard = intersection / (union + eps)  # [N, K+1]
    
    # Prune edges below threshold
    edge_weights = (jaccard >= jaccard_threshold).float()
    
    # Always keep self-connection
    edge_weights[:, 0] = 1.0
    
    return edge_weights

def _branch4_apply_adaptive_k_mask(raw_sim, adaptive_k, K_plus_1):
    """Apply per-sample adaptive K by masking out neighbors beyond each sample's K.
    
    Args:
        raw_sim: [N, K+1] similarity matrix
        adaptive_k: [N] per-sample K values
        K_plus_1: total number of neighbors including self
    Returns:
        masked_sim: [N, K+1] similarity with far neighbors zeroed out
    """
    N = raw_sim.shape[0]
    # Create position indices [0, 1, ..., K]
    positions = torch.arange(K_plus_1, device=raw_sim.device).unsqueeze(0).expand(N, -1)  # [N, K+1]
    # Mask: keep if position <= adaptive_k (position 0 = self)
    mask = (positions <= adaptive_k.unsqueeze(1)).float()  # [N, K+1]
    return raw_sim * mask
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # ============================================================
    # [Branch4] Adaptive KNN graph construction
    # ============================================================
    k_max_b4 = int(getattr(args, 'branch4_k_max', 30))
    k_min_b4 = int(getattr(args, 'branch4_k_min', 5))
    jaccard_thresh = float(getattr(args, 'branch4_jaccard_threshold', 0.0))
    
    # Search with k_max to have enough neighbors for adaptive selection
    effective_k = max(args.k_val, k_max_b4)
    D_mh, neighbors_mh = knn_search_pytorch_chunked(features, effective_k, num_heads=args.knn_heads)
    raw_sim = F.relu(D_mh).float()
    
    # Compute adaptive K per sample
    adaptive_k = _branch4_compute_adaptive_k(raw_sim, k_min_b4, effective_k)
    
    # Apply adaptive K masking
    raw_sim = _branch4_apply_adaptive_k_mask(raw_sim, adaptive_k, raw_sim.shape[1])
    
    # Apply Jaccard edge pruning based on candidate set overlap
    jaccard_weights = _branch4_jaccard_edge_pruning(
        neighbors_mh, static_cand_mask.float(), jaccard_thresh)
    raw_sim = raw_sim * jaccard_weights
    
    logger.info(f"   [Branch4] Adaptive graph: mean_k={adaptive_k.float().mean():.1f}, "
                 f"min_k={adaptive_k.min()}, max_k={adaptive_k.max()}, "
                 f"jaccard_thresh={jaccard_thresh}")
    ...
    with torch.no_grad():
        ...
        mask_expanded = mask_final.view(-1, 1).expand(-1, neighbors_mh.shape[1]).float()
