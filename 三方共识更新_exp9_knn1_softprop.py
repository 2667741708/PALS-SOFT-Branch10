...
python "三方共识更新_exp9_knn1_softprop.py" \
    ...
    --nr 0.2 \
    --out ./out_ultimate \
    --exp_name 三方共识更新_exp9_knn1_softprop/CUB200pr0.05nr0.2e250top_top_hl15_exp9_fuse_softpropdel0.5lsr0.5_23 \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
...
--dataset CIFAR100 \
--out topology_daes \
--exp_name "三方共识更新/pr0.05nr0.5e500top_exp_hl15_geofuse+softprop" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    for i in range(iterations):
        ...
        if i == 0:
            ...
            # [Branch3] Agreement-based Global Curriculum: 废除 epoch-ramp，改用全局共识率驱动权重
            ...
            if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
                ...
                # 计算全局一致率 (Model hard pred vs KNN soft argmax)
                ...
                w_model = max_w * (global_agreement ** consensus_power)
        ...
        if i == 0 and getattr(args, 'enable_knn1_geo_fuse', False) and (model_preds is not None):
            ...
            # [Branch3] Agreement-based weight: 用全局共识率代替 epoch-ramp
            global_agreement_geo = (model_preds.argmax(1) == knn1.argmax(1)).float().mean().item()
            consensus_power = float(getattr(args, 'consensus_power', 2.0))
            max_w_geo = float(getattr(args, 'knn1_geo_fuse_max_w', 1.0))
            w_model_geo = max_w_geo * (global_agreement_geo ** consensus_power)
            w_model_geo = float(np.clip(w_model_geo, 0.0, max_w_geo))

            # 3) log 空间融合并回投影
            log_knn = torch.log(knn1 + eps)
            log_geo = torch.log(geo + eps)
            log_fused = log_knn + w_model_geo * log_geo
