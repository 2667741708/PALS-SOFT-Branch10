"""
python exp9_softprop_daes信赖度_exp传播.py \
    --dataset Treeversity \
    --lpi 3 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播/Treeversity/lr0.01LPI3_Run_1del1_top_top_lsr0.0_k_15 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
--dataset CIFAR10 \
--out topology_daes \
--exp_name "exp9_softprop_daes信赖度_exp传播/pr0.5nr0.3e100top_top_hl15_exp8_knn1_softprop" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 [Epoch {epoch}] Reliable Selection - DAES Phase 1 + Prior Projection + EXP Phase 2")
    logger.info(f"{'='*80}")
    ...
    # 聚合得到基于多数邻居意见的 Phase 1 分布 [N, C]
    neighbor_soft_labels = curr_soft_out[neighbors_indices_only]
    phase1_dist = torch.einsum('nk,nkc->nc', daes_weights_norm, neighbor_soft_labels)

    # (保留之前的边权重控制逻辑) 计算节点自身的可信赖度
    self_preds = curr_soft_out.argmax(dim=1)
    reliability_scores = phase1_dist.gather(1, self_preds.unsqueeze(1)).squeeze(1)

    # ==============================================================================
    # 🟢 投影机制 (Projection): 生成 Phase 2 的净化输入
    # ==============================================================================
    # static_cand_mask 在 PLL 下是 0/1 掩码，在众包下是初始软分布权重
    cand_prior = static_cand_mask.float() 
    
    # 投影：邻居分数 × 自身先验候选集/软分布
    projected_dist = phase1_dist * cand_prior
    ...
    with torch.no_grad():
        ...
        logger.info(f"   ├─ 🚨 [Probe] Weight Override: {num_overridden} nodes ({(num_overridden/total_nodes)*100:.2f}%) yielded max weight to neighbors.")
    # ------------------------------------------------------------------------------

    # 执行全局的邻域意见聚合 (🚨注意：这里使用的是经过投影净化的 phase2_input！)
    ...
    # 📊 Stage 2: 最终结果日志
    logger.info(f"📊 [Stage 2] Final Selection (After Prior Projection + Weighted EXP)")
    logger.info(f"   ├─ Selected: {int(size_rel_2)} samples | Acc: {acc_rel_2*100:.2f}% | Overall Acc: {overall_acc_final*100:.2f}%")
    logger.info(f"   ├─ Change from Stage 0: Δsize={int(size_rel_2-size_init):+d}, Δacc={(acc_rel_2-acc_init)*100:+.2f}%")
    logger.info(f"{'='*80}\n")
