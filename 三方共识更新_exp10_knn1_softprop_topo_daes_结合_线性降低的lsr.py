...
# CUB200 示例（不开启模型融合）
python "三方共识更新_exp10_knn1_softprop_topo_daes_结合_线性降低的lsr.py" \
    --dataset CUB200 \
    --train_root ./data \
    --lpi 10 \
    --pr 0.05 \
    --nr 0.2 \
    --out ./topology_daes \
    --exp_name 三方共识更新_exp10_knn1_softprop_topo_daes_结合_线性降低的lsr/CUB200pr0.05nr0.2e250exp_topdaes_hl15_del0.5lsr0.0_23 \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
...
--dataset CIFAR100 \
--out topology_daes \
--exp_name "三方共识更新_exp10_knn1_softprop_topo_daes_结合_线性降低的lsr/pr0.05nr0.5e500topdaes_topdaes_hl15_geofuse+softprop23lsr0.5" \
...
--dataset CIFAR10 \
--out topology_daes \
--exp_name "三方共识更新_exp10_knn1_softprop_topo_daes_结合_线性降低的lsr/ cifar10pr0.5nr0.3e100topdaes_topdaes_geofuse+softprop_lsr0_0_直接融" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
...
def get_current_lsr(epoch, total_epochs, start_lsr=0.5, end_lsr=0.0):
    """计算当前 epoch 的线性衰减 LSR 分数"""
    if total_epochs <= 1:
        return start_lsr
    # 随进度线性下降：start -> end
    fraction = epoch / (total_epochs - 1)
    current_lsr = start_lsr + fraction * (end_lsr - start_lsr)
    return max(0, current_lsr)
...
# ==============================================================================
        # [EXP6 升级版: Uncertainty-Aware Dynamic Gating (方案一)]
        # ==============================================================================
        if i == 0 and getattr(args, 'enable_knn1_geo_fuse', False) and (model_preds is not None):
            eps = float(getattr(args, 'knn1_geo_fuse_eps', 1e-8))
            max_w = float(getattr(args, 'knn1_geo_fuse_max_w', 1.0))

            # epoch: 0-indexed; 让最后一个 epoch 的权重达到 max_w (时间调度)
            denom = float(max(getattr(args, 'epochs', 1) - 1, 1))
            w_model = max_w * float(epoch) / denom
            w_model = float(np.clip(w_model, 0.0, max_w))
            ...
            # --- 🚀 [核心修改] 节点级不确定性感知的动态门控 ---
            # 计算 KNN 分布的归一化熵 (H_knn) [N, 1]
            entropy_knn = -torch.sum(knn1 * torch.log(knn1 + eps), dim=1, keepdim=True)
            norm_entropy_knn = entropy_knn / math.log(args.num_classes)
            
            # 动态决定几何分数(Model)的权重：
            # 时间上限 (w_model) × 样本的 KNN 混乱度 (norm_entropy_knn)
            # 如果 KNN 很自信(熵极低)，即使在训练后期，也尽量保留 KNN 的原始意见；
            # 如果 KNN 很混乱(熵高)，则大幅听取邻域模型的共识意见。
            w_geo_dynamic = w_model * norm_entropy_knn 
            
            # 线性加权融合 (Linear Pooling)
            fused = (1.0 - w_geo_dynamic) * knn1 + w_geo_dynamic * geo
            ...
            if logger is not None and epoch % 10 == 0:
                avg_dynamic_w = w_geo_dynamic.mean().item()
                logger.info(f" 🧪 [EXP6 V2] Uncertainty-Aware Fusion | Global w_model={w_model:.3f} | Actual Avg w_geo={avg_dynamic_w:.3f}")
            
            # 📊 Stage 1.5: 融合后结果统计... (保留你原有的 Stage 1.5 统计代码)
            with torch.no_grad():
                mask_exp6, pred_exp6, acc_exp6, size_exp6 = _filter_logic(curr_soft_out)
                overall_pred_exp6 = curr_soft_out.argmax(dim=1)
                overall_acc_exp6 = (overall_pred_exp6 == clean_labels).float().mean().item()
                logger.info(f"📊 [Stage 1.5] After Dynamic Gating Fusion")
                logger.info(f"   ├─ Selected: {int(size_exp6)} samples | Acc: {acc_exp6*100:.2f}% | Overall Acc: {overall_acc_exp6*100:.2f}%")
                logger.info(f"   ├─ Change from Stage 1: Δsize={int(size_exp6-size_knn1):+d}, Δacc={(acc_exp6-acc_knn1)*100:+.2f}%")
    # 3. 最终筛选执行
...
def run_single_experiment(args):
    ...
    for epoch in range(total_epochs):
        ...
        sampler = WeightedRandomSampler(weights=sampling_weights_aligned, num_samples=total_target, replacement=True)
        unified_loader = DataLoader(unified_dataset, batch_size=args.batch_size, sampler=sampler, num_workers=args.num_workers, pin_memory=True, drop_last=True)
        # 在 for epoch in range(total_epochs): 循环内
        current_lsr_val = get_current_lsr(epoch, total_epochs, start_lsr=0.5, end_lsr=0.0)
        logger.info(f" 📉 [Dynamic LSR] Current Smoothing Rate: {current_lsr_val:.4f}")
        # ==============================================================================
        # 5.6 🚀 执行混合单流训练
        # ==============================================================================
