...
def iterative_pseudo_labelling(features, partial_labels, k, iterations, device):
    try: res = faiss.StandardGpuResources(); index = faiss.GpuIndexFlatIP(res, features.shape[1])
    except Exception: index = faiss.IndexFlatIP(features.shape[1])
    
    # (确保 features 是 float32)
    index.add(features.cpu().numpy().astype(np.float32)) 
    sim, neighbor_idx = index.search(features.cpu().numpy().astype(np.float32), k + 1)
    
    # --- (关键修复) ---
    # 1. 确保 sim 是 torch.float
    sim = torch.from_numpy(sim[:, 1:]).float().to(device) 
    # 2. 确保 neighbor_idx 是 torch.long
    neighbor_idx = torch.from_numpy(neighbor_idx[:, 1:]).long().to(device) 
    # 3. 确保 current_candidates 是 torch.float
    current_candidates = partial_labels.to(device).float() 
    # --- (修复结束) ---

    for i in range(iterations):
        weighted_votes = torch.einsum('nk,nkc->nc', sim, current_candidates[neighbor_idx])
    ...
    for i in range(num_batches):
        ...
        if iter_unsup:
            ...
            with torch.no_grad(), autocast('cuda'):
                ...
                p_s = F.softmax(classifier(encoder(unsup_s)), dim=1)
                ...
                final_weights = softmatch_weights
        ...
        # --- 3. 全局一致性正则化 (T-Rept KL) ---
        loss_consist = torch.tensor(0.0, device=device)
        
        if all_weak_imgs: # (Check if the list is not empty)
            all_w = torch.cat(all_weak_imgs)
            all_s = torch.cat(all_strong_imgs)
            current_batch_size = all_w.shape[0]
            
            with autocast('cuda'):
                ## --- 🚀 MODIFICATION: Use encoder --- ##
                logits_w = classifier(encoder(all_w))
                logits_s = classifier(encoder(all_s))
                
                T = 3.0
                logp_s = F.log_softmax(logits_s / T, dim=1)
                p_w    = F.softmax(logits_w / T, dim=1)
                kl_loss = F.kl_div(logp_s, p_w.detach(), reduction='batchmean') * (T ** 2)
                loss_consist = (1.0 * kl_loss)
                                   
            total_loss_consist += loss_consist.item() * current_batch_size

        # --- 4. 总损失与反向传播 ---
        total_loss = (loss_s + 
                      dynamic_consistency_weight * loss_c + 
                      args.feature_consistency_weight * loss_consist)
...
import torch.nn.functional as F
import numpy as np
import faiss  # 必须导入 faiss

def reliable_pseudolabel_selection_weighted(logger, args, device, trainloader, features, epoch, model_preds=None):
    
    # ==============================================================================
    # 🛠️ Helper 1: 基于 FAISS 的极速多头 KNN (无需手动分块)
    # ==============================================================================
    def multi_head_knn_search_faiss(feats, k, num_heads=4):
        """
        利用数学等价性实现 FAISS 多头搜索。
        原理: 拼接后的[分头归一化]向量的点积 == 多头余弦相似度之和。
        """
        # 1. 确保特征在 GPU 上，方便进行 Tensor 变换
        ...
        # 2. [关键步骤] 分头 + 独立归一化
        # [N, D] -> [N, H, D/H]
        feats_multi = feats.view(N, num_heads, head_dim)
        # 对每个头(dim=2)进行 L2 归一化
        feats_multi = F.normalize(feats_multi, p=2, dim=2)
        
        # 3. 拼回长向量 [N, D] 并转为 CPU Numpy
        # 此时的向量已经不是原始特征了，而是预处理过的“多头拼接”特征
        feats_reshaped = feats_multi.reshape(N, D_dim).cpu().numpy().astype(np.float32)
        
        # 4. FAISS 极速搜索 (Inner Product)
        # Inner Product of Normalized Sub-vectors = Sum of Cosine Similarities
        if logger: logger.info(f"   >>> [Selection] Building FAISS Index (Dim={D_dim}, Heads={num_heads})...")
        
        index = faiss.IndexFlatIP(D_dim) 
        index.add(feats_reshaped)
        
        # 搜索 K+1 个邻居
        D_faiss, I_faiss = index.search(feats_reshaped, k+1)
        
        # 5. 数值调整: Sum -> Mean
        # FAISS 返回的是 H 个头的相似度总和，我们需要平均值
        D_faiss = D_faiss / float(num_heads)
        
        # 6. 转回 GPU 并提升精度为 Double (Float64) 以对接后续逻辑
        return (torch.from_numpy(D_faiss).to(device).double(), 
                torch.from_numpy(I_faiss).to(device).long())
    ...
    # ==============================================================================
    # 使用 Helper 1 进行搜索
    D_mh, neighbors_mh = multi_head_knn_search_faiss(features, args.k_val, num_heads=4)
    ...
    return (
        final_selection_mask.cpu().float(), 
        selected_labels_val.cpu(), 
        final_pl.cpu(), 
        torch.max(model_preds, 1)[1].cpu(), 
        final_soft.cpu().float()
    )
...
# --- 🚀 混合方案：根据数据集类型估计目标分布 ---
    ...
    for epoch in range(args.epochs):
        ...
        # ==========================================================
        # --- 3.4 创建 DataLoaders (修改版) ---
        # ==========================================================
        loader_sup, loader_unsup = None, None
            
        # 1. 可靠集 Loader (保持不变：仅包含筛选出的可靠样本)
        if rel_indices:
            ds_sup = SSLReadyDataset(base_train_ds, list(rel_indices), list(rel_labels), weak_t, strong_t)
            # drop_last=True 防止 batchnorm 在最后一个小 batch 出错
            loader_sup = DataLoader(ds_sup, batch_size=args.batch_size, shuffle=True, 
                                    num_workers=args.num_workers, pin_memory=True, drop_last=True)
        
        # 2. 无监督 Loader (修改：使用【全部】样本，而非仅不可靠样本)
        # 原代码: if unreliable_indices: ...
        # 新代码: 直接使用全部索引
        all_indices = list(range(len(base_train_ds)))
        
        # 注意：这里 label 传入 [-1] 代表我们不使用它的真实标签，而是依靠模型预测
        ds_unsup = SSLReadyDataset(base_train_ds, all_indices, [-1] * len(all_indices), weak_t, strong_t)
        
        # Batch Size 可以沿用 args.batch_size 或 args.unreliable_batch_size
        loader_unsup = DataLoader(ds_unsup, batch_size=args.batch_size, shuffle=True, 
                                  num_workers=args.num_workers, pin_memory=True, drop_last=True)

        logger.info(f" -> Loaders Ready: Sup (Reliable Only, N={len(rel_indices)}) | Unsup (All Data, N={len(all_indices)})")
        # --- 3.5 调用训练循环 ---
        # (现在我们传递 knn_scores)
...
#exp D
"""python trept_unsup_mixup_crowd_pals_TPDS_standard_rank_calibrated_differ_exp_delta_MH_KNN_high_index_topology_unified.py \
    --dataset CIFAR10 \
    --train_root ./data \
    --lpi 10 \
    --pr 0.5 --nr 0.3\
    --out ./out_ultimate \
    --exp_name PALS_softMatch/trept_unsup_mixup_crowd_pals_TPDS_standard_rank_calibrated_differ_exp_delta_MH_KNN_high_index_topology_unified/CIFAR10/consensus_power2/exp_daes/delta0.25_model_predict_123_p0.5n0.3_bs256_rand_lam_no_cross_mixup_noncensus1.0_rectify \
    --batch_size 256 \
    --lr 0.1 \
    --wd 1e-3 \
    --feature_consistency_weight 0.0 --consistency_weight 1.0 \
    ...
    --delta 0.25 \
    --network R18 \
    --epochs  500 --cuda_dev 0 --sim_mode_1 topology_entropy --sim_mode_2 daes --num_worker 4        """      
...
python trept_unsup_mixup_crowd_pals_TPDS_standard_rank_calibrated_differ_exp_delta_MH_KNN_high_index_topology_unified.py \
    ...
    --pr 0.05 --nr 0.2\
    --out ./out_ultimate \
    --exp_name PALS_softMatch/trept_unsup_mixup_crowd_pals_TPDS_standard_rank_calibrated_differ_exp_delta_MH_KNN_high_index_topology_unified/CUB200/topo_daes/delta0.5_model_predict_23_p0.05n0.2_bs64 \
    --batch_size 64 \
    --lr 0.05 \
    --wd 5e-4 \
    --feature_consistency_weight 0.0 --consistency_weight 1.0 \
    ...
    --network R18 \
    --epochs  250 \
    --cuda_dev 1 --sim_mode_1 topology_entropy --sim_mode_2 daes --num_worker 2          
"""      