...
python 三方共识更新_exp9_softprop精简版_self_node.py \
    ...
    --lpi 3 \
    --out ./topology_daes \
    --exp_name 三方共识更新_exp9_softprop精简版_self_node/Treeversity/LPI3_Run_1del1_top_top_lsr0.0_k_5 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
    ...
    for i in range(iterations):
        ...
        if i == 0:
            ...
            if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
                ...
                # 1) 模型预测融合: 包含自身预测的加权平均
                neighbor_idx = neighbors_mh  # 取消 [:, 1:]
                w_raw = raw_sim              # 取消 [:, 1:]
        ...
        # 排除自身权重 (col 0),只用邻居投票
        # 2. 邻居投票 (Message Passing)
        # 保留自身权重 (col 0)，执行 K+1 个节点的完整聚合
        voting_weights = refined_sim  # 取消 [:, 1:]
        
        # 同步传入包含自身的 K+1 个标签
        neighbor_vals = propagation_input[neighbors_mh]  # 取消 [:, 1:]
