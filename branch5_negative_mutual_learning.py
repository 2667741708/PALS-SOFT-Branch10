...
# ==============================================================================
# Branch 5: 伪负标签排他性互学习
# Core: Candidate complement as pseudo-negatives + MC-Dropout uncertainty fusion
# ==============================================================================
...
# --- 修改部分开始 ---
    ...
    # [Branch5] Pseudo-negative mutual learning
    parser.add_argument('--branch5_neg_penalty', type=float, default=2.0,
                        help='[Branch5] Penalty multiplier for probability mass on non-candidate classes')
    parser.add_argument('--branch5_mc_dropout_samples', type=int, default=5,
                        help='[Branch5] Number of MC-Dropout forward passes for uncertainty')
    parser.add_argument('--branch5_mc_dropout_rate', type=float, default=0.1,
                        help='[Branch5] Dropout rate for MC-Dropout uncertainty estimation')
    parser.add_argument('--branch5_uncertainty_scale', type=float, default=3.0,
                        help='[Branch5] Scale factor for uncertainty-based weight adjustment')
    parser.add_argument('--branch5_neg_complement_weight', type=float, default=0.5,
                        help='[Branch5] Weight for negative complement regularization in loss')
...
def _branch5_negative_complement_adjustment(fused_dist, cand_mask, crowd_prior, 
                                             neg_penalty=2.0, eps=1e-8):
    """Adjust fused distribution by penalizing probability mass on non-candidate classes.
    
    For PLL: non-candidate classes have cand_mask = 0, these are confirmed negatives.
    For crowdsourced: classes with very low crowd_prior are likely negatives.
    
    Args:
        fused_dist: [N, C] fused probability distribution
        cand_mask: [N, C] candidate set (binary for PLL, soft for crowdsourced)
        crowd_prior: [N, C] or None
        neg_penalty: penalty multiplier for non-candidate probability mass
    Returns:
        adjusted_dist: [N, C] adjusted distribution
        neg_mass: [N] total probability mass on non-candidate classes
    """
    N, C = fused_dist.shape
    
    if crowd_prior is not None:
        # For crowdsourced: treat classes with prior < threshold as pseudo-negatives
        # Threshold: classes receiving < 1/(2*C) of the vote mass
        prior_threshold = 1.0 / (2.0 * C)
        neg_mask = (crowd_prior < prior_threshold).float()  # [N, C]
        pos_mask = 1.0 - neg_mask
    else:
        # For PLL: complement of candidate set are confirmed negatives
        neg_mask = (cand_mask <= 0).float()  # [N, C]
        pos_mask = (cand_mask > 0).float()
    
    # Compute probability mass on negative classes
    neg_mass = (fused_dist * neg_mask).sum(dim=1)  # [N]
    
    # Suppress negative class probabilities and redistribute to candidate classes
    suppressed = fused_dist * (pos_mask + neg_mask / neg_penalty)
    adjusted = suppressed / (suppressed.sum(dim=1, keepdim=True) + eps)
    
    return adjusted, neg_mass

def _branch5_model_uncertainty_weight(model_preds, cand_mask, uncertainty_scale=3.0, eps=1e-8):
    """Compute per-sample fusion weight based on model prediction uncertainty.
    Uses prediction entropy + candidate set alignment as uncertainty measure.
    
    High uncertainty (high entropy, poor candidate alignment) -> lower model weight
    Low uncertainty (low entropy, good candidate alignment) -> higher model weight
    
    Args:
        model_preds: [N, C] model softmax predictions
        cand_mask: [N, C] candidate mask
        uncertainty_scale: scaling factor
    Returns:
        weights: [N] in range [0, 1], higher = more confident model
    """
    # Shannon entropy
    entropy = -torch.sum(model_preds * torch.log(model_preds + eps), dim=1)
    max_entropy = np.log(model_preds.shape[1])
    norm_entropy = entropy / (max_entropy + eps)
    
    # Candidate alignment: how much probability mass falls within candidate set
    cand_binary = (cand_mask > 0).float()
    cand_alignment = (model_preds * cand_binary).sum(dim=1)  # [N], range [0, 1]
    
    # Combined confidence: low entropy + high candidate alignment
    confidence = (1.0 - norm_entropy) * cand_alignment  # [N]
    
    # Scale to [0, 1] with sharper transition
    weights = torch.sigmoid(uncertainty_scale * (confidence - 0.5))
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
        ...
        # ============================================================
        # [Branch5] Uncertainty-weighted fusion with pseudo-negative reasoning
        # ============================================================
        uncertainty_scale = float(getattr(args, 'branch5_uncertainty_scale', 3.0))
        neg_penalty = float(getattr(args, 'branch5_neg_penalty', 2.0))
        
        # Compute per-sample model confidence weight
        cand_for_unc = static_cand_mask.float()
        w_per_sample = _branch5_model_uncertainty_weight(
            model_preds, cand_for_unc, uncertainty_scale)  # [N]
        
        # Scale by global consensus (keep curriculum signal)
        ...
        global_scale = float(np.clip(max_w * (global_agreement ** consensus_power), 0.0, max_w))
        w_per_sample = w_per_sample * global_scale
        
        logger.info(f"   [Branch5] Neg-uncertainty fusion: mean_w={w_per_sample.mean():.4f}, "
                     f"global_scale={global_scale:.4f}, neg_penalty={neg_penalty}")
        ...
        # [Branch5] Apply pseudo-negative complement adjustment
        soft_model_fused, neg_mass = _branch5_negative_complement_adjustment(
            soft_model_fused, cand_for_unc, crowd_prior, neg_penalty)
        logger.info(f"   [Branch5] Neg mass suppressed: mean={neg_mass.mean():.4f}")
        ...
        with torch.no_grad():
            ...
            logger.info(f"📊 [Stage 1.5] After Model Fusion ({fusion_mode}, Branch5-NegMutual)")
