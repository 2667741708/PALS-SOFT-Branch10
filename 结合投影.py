...
python 结合投影.py \
    ...
    --lpi 3 \
    --out ./topology_daes \
    --exp_name 结合投影/Treeversity/LPI3_Run_1del1_exp_top \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
python 结合投影.py \
    ...
    --lpi 3 \
    --out ./topology_daes \
    --exp_name 结合投影/Treeversity/LPI3_Run_1del1_top_daes_top_daesdel_1.0_lsr0.0 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
--dataset CIFAR10 \
--out topology_daes \
--exp_name "三方共识更新/pr0.05nr0.4e500top_top_hl15_exp6_knn1_geofuse" \
--enable_dual_source_refinement \
--enable_dynamic_sampling \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"🚀 [Epoch {epoch}] Reliable Selection - MultiMethod Phase1 + Crowd-Aware Phase2 [{args.sim_mode_1}→{args.sim_mode_2}]")
    logger.info(f"{'='*80}")
    ...
    # ==============================================================================
    # 🔴 第一阶段 (Phase 1): 多方法 KNN 软分布聚合对比
    # 以 curr_soft_out（当前众包加权软标签）作为邻居意见的输入
    # ==============================================================================
    neighbor_soft_all = curr_soft_out[neighbors_mh[:, 1:]]  # [N, K, C]
    
    def _phase1_probe(mode_name):
        """用指定方法对 curr_soft_out 做一次无梯度 KNN 聚合，返回聚合后准确率"""
        with torch.no_grad():
            sim = get_weight_matrix(mode_name, raw_sim, neighbors_mh, curr_soft_out, args)
            w = sim[:, 1:]
            w = w / (w.sum(dim=1, keepdim=True) + 1e-12)
            agg = torch.einsum('nk,nkc->nc', w, neighbor_soft_all)
            acc = (agg.argmax(dim=1) == clean_labels).float().mean().item()
        return agg, acc

    probe_methods = ['exp', 'daes', 'topology', 'topology_daes']
    probe_results = {}
    for m in probe_methods:
        agg_dist, probe_acc = _phase1_probe(m)
        probe_results[m] = (agg_dist, probe_acc)

    # 用 sim_mode_1 指定的方法作为真正的第一阶段聚合结果
    phase1_knn_dist, acc_phase1 = probe_results[args.sim_mode_1]

    # 日志：打印所有方法的 Phase 1 准确率对比
    logger.info(f"📊 [Phase 1 Probe] KNN Acc comparison (input: soft crowd labels):")
    for m in probe_methods:
        marker = " ★" if m == args.sim_mode_1 else "  "
        logger.info(f"   {marker}├─ {m:<15}: {probe_results[m][1]*100:.2f}%")

    # ==============================================================================
    # 🟢 融合 + 投影（Weighted GeoMean Fusion & Projection）
    # ==============================================================================
    ...
    w_model = float(np.clip(max_w * float(epoch) / total_epochs, 0.0, max_w))
    ...
    geo_mean_norm = torch.exp(log_fused)
    geo_mean_norm = geo_mean_norm / (geo_mean_norm.sum(dim=1, keepdim=True) + 1e-12)
    ...
    # 探针 2：融合投影后准确率
    with torch.no_grad():
        acc_proj = (phase2_input.argmax(dim=1) == clean_labels).float().mean().item()

    # ==============================================================================
    # 🔵 第二阶段 (Phase 2): sim_mode_2 驱动的信赖度加权传播
    # ==============================================================================
    refined_sim_2 = get_weight_matrix(args.sim_mode_2, raw_sim, neighbors_mh, phase2_input, args)
    
    # 注入投影信赖度分数到边权重
    mapped_reliabilities = reliability_scores[neighbors_mh]
    adjusted_weights = refined_sim_2 * mapped_reliabilities
    
    # 排除自身权重进行投票分配
    voting_weights_2 = adjusted_weights[:, 1:]
    voting_weights_2 = voting_weights_2 / (voting_weights_2.sum(dim=1, keepdim=True) + 1e-12)
    self_weight = adjusted_weights[:, 0]

    # 探针：动态夺权统计
    with torch.no_grad():
        num_overridden = (voting_weights_2.max(dim=1)[0] > self_weight).sum().item()
        total_nodes = N

    # 执行最终聚合（邻居 + 自身）
    neighbor_vals_all = phase2_input[neighbors_mh[:, 1:]]
    weighted_votes = torch.einsum('nk,nkc->nc', voting_weights_2, neighbor_vals_all)
    weighted_votes = weighted_votes + phase2_input * self_weight.unsqueeze(1)
    ...
    logger.info(f"📊 [Stage Summary] Phase1({args.sim_mode_1})→GeoMean+Proj→Phase2({args.sim_mode_2})")
    logger.info(f"   ├─ 1. Phase 1 ({args.sim_mode_1}) Acc: {acc_phase1*100:.2f}%")
    logger.info(f"   ├─ 2. GeoMean+Projected Acc: {acc_proj*100:.2f}% (Rel via {'Anchor' if is_crowd_data else 'Sum'}, w_model={w_model:.3f})")
    logger.info(f"   ├─ 3. Phase 2 ({args.sim_mode_2}) Final Acc: {overall_acc_final*100:.2f}%")
    logger.info(f"   ├─ 🚨 Weight Override: {num_overridden} nodes ({(num_overridden/total_nodes)*100:.2f}%)")
    logger.info(f"   └─ Final Reliable Set: {int(size_rel_2)} samples | Acc: {acc_rel_2*100:.2f}%")
    logger.info(f"{'='*80}\n")
