...
def apply_op(img, op_name, magnitude, fillcolor):
    if op_name == "shearX":
...
class SubPolicy(object):
    ...
    def __call__(self, img):
        if random.random() < self.p1: 
            img = apply_op(img, self.operation1_name, self.magnitude1, self.fillcolor)
        if random.random() < self.p2: 
            img = apply_op(img, self.operation2_name, self.magnitude2, self.fillcolor)
        return img