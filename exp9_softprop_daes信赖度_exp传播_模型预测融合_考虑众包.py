"""
python exp9_softprop_daes信赖度_exp传播_模型预测融合_考虑众包.py \
    --dataset Treeversity \
    --lpi 3 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播_模型预测融合_考虑众包/Treeversity/lr0.01LPI3_Run_1del1_top_top_lsr0.0_k_15_new \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # 1. 投影到候选集 (逐元素乘法)
    projected_dist = geo_mean_norm * cand_prior  # [N, C]

    # 2. 判断是否为众包数据集（检查是否存在 weights）
    is_crowd_source = hasattr(trainloader.dataset, 'weights')

    if is_crowd_source:
        # ========== 众包数据集：加权计算 ==========
        # 获取归一化的众包权重 [N, C]
        crowd_weights = torch.tensor(
            trainloader.dataset.weights, 
            device=device, 
            dtype=torch.float32
        )
        ...
        # 可靠度 = 融合意见与加权候选集的期望重叠
        # 高权重标签上的概率质量贡献更多可靠度
        weighted_projected = projected_dist * crowd_weights  # 加权
        reliability_scores = weighted_projected.sum(dim=1)    # 加权求和
        
        # 归一化因子也用加权版本（保持数值一致性）
        norm_factor = (cand_prior * crowd_weights).sum(dim=1).clamp(min=1e-12)
        
    else:
        # ========== 普通数据集：等同权重（原始代码 B 逻辑）==========
        reliability_scores = projected_dist.sum(dim=1)
        norm_factor = cand_prior.sum(dim=1).clamp(min=1e-12)

    # 3. 第二次归一化，生成下一阶段的输入
    # 注意：除以 reliability_scores（而非 norm_factor），保持物理意义一致性
