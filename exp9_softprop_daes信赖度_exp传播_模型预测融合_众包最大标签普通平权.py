"""
python exp9_softprop_daes信赖度_exp传播_模型预测融合_众包最大标签普通平权.py \
    --dataset Treeversity \
    --lpi 3 \
    --out ./topology_daes \
    --exp_name exp9_softprop_daes信赖度_exp传播_模型预测融合_众包最大标签普通平权/Treeversity/lr0.01LPI3_Run_1del1_top_top_lsr0.0_k_15_new \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    # 核心逻辑：区分“硬掩码约束”与“先验权重引导”
    if args.dataset in ['Treeversity', 'Benthic', 'Plankton']:
        # 众包模式：考虑到先验权重不平等，引入加权映射
        # 不仅仅是 sum，而是计算投影分布与原始分布的互信息或加权对齐度
        # 代码 A 的变体：关注概率质量在先验高权区的集中度
        prior_weights = static_cand_mask.float() 
        reliability_scores = torch.sum(geo_mean_norm * prior_weights, dim=1)
    else:
        # 普通 PL 模式：标签地位平等，使用代码 B 的广义投影
