...
python 结合投影_众包_自节点.py \
    ...
    --lpi 3 \
    --out ./topology_daes \
    --exp_name 结合投影_众包_自节点/Benthic/fold2LPI3/del1.0_lsr0.0k5_单头_可投到众包_hl100 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
python 结合投影_众包_自节点.py \
    ...
    --lpi 10 \
    --out ./topology_daes \
    --exp_name 结合投影_众包_自节点/Benthic/fold2LPI10/del1.0_lsr0.0k5_单头_可投到众包_hl100_123_nofuse_nofold3 \
    --network R50 \
    --epochs 100 \
    --batch_size 32 \
    --lr 0.05 \
...
"""
# ==============================================================================
# Branch 2: 面向细粒度视觉分类的层次化联合证据融合
# Core: Coarse-level KNN consistency filter + Dirichlet evidence fusion
# ==============================================================================
...
# --- 修改部分开始 ---
    ...
    # [Branch2] Hierarchical evidence fusion
    parser.add_argument('--branch2_coarse_classes', type=int, default=20,
                        help='[Branch2] Number of coarse superclasses for hierarchical grouping')
    parser.add_argument('--branch2_coarse_filter_thresh', type=float, default=0.5,
                        help='[Branch2] Threshold for coarse consistency filtering of KNN neighbors')
    parser.add_argument('--branch2_evidence_scale', type=float, default=10.0,
                        help='[Branch2] Scale factor for Dirichlet evidence computation')
    parser.add_argument('--branch2_uncertainty_blend', type=float, default=0.5,
                        help='[Branch2] Blend factor: 0=trust KNN, 1=trust model based on uncertainty')
...
def get_topology_guided_affinity(raw_D, neighbors_indices, current_soft_labels, num_classes, rel_mode='masked_entropy', gamma=2.0, eps=1e-12):
...
def get_topology_daes_affinity(raw_D, neighbors_indices, current_soft_labels, args):
    ...
    if rel_mode == 'masked_entropy':
        ...
        norm_score = -torch.sum(p_self * torch.log(p_knn + eps), dim=1) / (np.log(num_classes) + eps)
    elif rel_mode == 'agree':
        agree_mass = torch.sum(p_knn * p_self, dim=1).clamp(min=eps, max=1.0)
        norm_score = (-torch.log(agree_mass)) / (np.log(num_classes) + eps)
...
def _branch2_build_coarse_mapping(num_classes, num_coarse):
    """Build a deterministic mapping from fine classes to coarse superclasses.
    For CIFAR-100, this matches the standard 20 superclass grouping.
    For other datasets, uses uniform partitioning."""
    mapping = torch.zeros(num_classes, dtype=torch.long)
    classes_per_group = max(1, num_classes // num_coarse)
    for i in range(num_classes):
        mapping[i] = min(i // classes_per_group, num_coarse - 1)
    return mapping

def _branch2_coarse_consistency_filter(neighbor_idx, model_preds, knn_raw, raw_sim, 
                                        coarse_mapping, threshold, device):
    """Filter KNN neighbors based on coarse-level consistency with model predictions.
    Returns refined similarity weights where inconsistent neighbors are downweighted."""
    N, K = neighbor_idx.shape
    # Get coarse predictions from model
    num_coarse = coarse_mapping.max().item() + 1
    coarse_mapping = coarse_mapping.to(device)
    
    # Model's coarse prediction: aggregate fine-grained probs into coarse groups
    # model_preds: [N, C_fine]
    C_fine = model_preds.shape[1]
    # Build aggregation matrix [C_fine, C_coarse]
    agg_matrix = torch.zeros(C_fine, num_coarse, device=device)
    for c in range(C_fine):
        agg_matrix[c, coarse_mapping[c]] = 1.0
    
    model_coarse = torch.mm(model_preds, agg_matrix)  # [N, C_coarse]
    model_coarse_pred = model_coarse.argmax(dim=1)  # [N]
    
    # KNN neighbor coarse predictions
    knn_coarse = torch.mm(knn_raw, agg_matrix)  # [N, C_coarse] - but we need per-neighbor
    # Actually we need each neighbor's coarse prediction
    neighbor_fine_preds = model_preds[neighbor_idx]  # [N, K, C_fine]
    neighbor_coarse_preds = torch.einsum('nkc,cd->nkd', neighbor_fine_preds, agg_matrix)  # [N, K, C_coarse]
    neighbor_coarse_labels = neighbor_coarse_preds.argmax(dim=2)  # [N, K]
    
    # Check if each neighbor agrees at coarse level with the center node
    center_coarse = model_coarse_pred.unsqueeze(1).expand(-1, K)  # [N, K]
    coarse_agree = (neighbor_coarse_labels == center_coarse).float()  # [N, K]
    
    # Soft filtering: downweight disagreeing neighbors
    filter_weight = torch.where(coarse_agree > 0, 
                                torch.ones_like(coarse_agree),
                                torch.full_like(coarse_agree, 1.0 - threshold))
    
    refined_sim = raw_sim * filter_weight
    return refined_sim

def _branch2_dirichlet_uncertainty(probs, scale=10.0, eps=1e-8):
    """Compute Dirichlet-based uncertainty from probability distributions.
    Returns uncertainty score in [0, 1] where 1 = maximum uncertainty."""
    # evidence = scale * probs (treating probs as parameters)
    evidence = scale * probs + eps
    S = evidence.sum(dim=1, keepdim=True)  # Dirichlet strength
    uncertainty = probs.shape[1] / S.squeeze(1)  # vacuity
...
def reliable_pseudolabel_selection_advanced(logger, args, device, trainloader, features, epoch, 
    ...
    if getattr(args, 'enable_knn1_model_fuse', False) and (model_preds is not None):
        ...
        # ============================================================
        # [Branch2] Hierarchical coarse-level consistency filtering
        # ============================================================
        num_coarse = int(getattr(args, 'branch2_coarse_classes', 20))
        coarse_thresh = float(getattr(args, 'branch2_coarse_filter_thresh', 0.5))
        coarse_mapping = _branch2_build_coarse_mapping(args.num_classes, num_coarse)
        
        # Apply coarse consistency filter to KNN weights
        w_filtered = _branch2_coarse_consistency_filter(
            neighbor_idx, model_preds, knn1_raw, w_raw,
            coarse_mapping, coarse_thresh, device)
        
        w_norm = w_filtered / (w_filtered.sum(dim=1, keepdim=True) + eps)
        ...
        # ============================================================
        # [Branch2] Dirichlet evidence-based uncertainty fusion
        # ============================================================
        evidence_scale = float(getattr(args, 'branch2_evidence_scale', 10.0))
        u_blend = float(getattr(args, 'branch2_uncertainty_blend', 0.5))
        
        # Compute uncertainty for both sources
        u_knn = _branch2_dirichlet_uncertainty(knn1_raw, evidence_scale)  # [N]
        u_model = _branch2_dirichlet_uncertainty(geo, evidence_scale)  # [N]
        
        # Adaptive per-sample weight: trust whichever source is more certain
        # w -> 1 means trust model more, w -> 0 means trust KNN more
        w_per_sample = u_blend * (u_knn / (u_knn + u_model + eps))  # [N]
        w_per_sample = w_per_sample + (1 - u_blend) * float(np.clip(
            max_w * ((model_preds.argmax(1) == knn1_raw.argmax(1)).float().mean().item() ** 
                     float(getattr(args, 'consensus_power', 2.0))), 0.0, max_w))
        w_per_sample = torch.clamp(w_per_sample, 0.0, max_w)
        
        logger.info(f"   [Branch2] Hierarchical fusion: mean_w={w_per_sample.mean():.4f}, "
                     f"u_knn={u_knn.mean():.4f}, u_model={u_model.mean():.4f}, "
                     f"coarse_classes={num_coarse}")
        
        fusion_mode = getattr(args, 'fusion_mode', 'weighted_sum')
        w_sample_expanded = w_per_sample.unsqueeze(1)
        
        if fusion_mode == 'geometric':
            log_knn = torch.log(knn1_raw + eps)
            log_geo = torch.log(geo + eps)
            log_fused_temp = log_knn + w_sample_expanded * log_geo
            soft_model_fused = F.softmax(log_fused_temp, dim=1)
        else:
            soft_model_fused = knn1_raw + w_sample_expanded * geo
        ...
        with torch.no_grad():
            ...
            logger.info(f"📊 [Stage 1.5] After Model Fusion ({fusion_mode}, Branch2-Hierarchical)")
        ...
        if crowd_prior is not None:
            if fusion_mode == 'geometric':
                log_fused = log_knn + w_sample_expanded * log_geo
    ...
    # 再次归一化作为第二次迭代的输入
    propagation_input_2 = base_dist / (base_dist.sum(dim=1, keepdim=True) + eps)
    
    with torch.no_grad():
        mask_prior, pred_prior, acc_prior, size_prior = _filter_logic(propagation_input_2)
        overall_pred_prior = propagation_input_2.argmax(dim=1)
        overall_acc_prior = (overall_pred_prior == clean_labels).float().mean().item()
...
def run_single_experiment(args):
    ...
    if args.dataset in ['CIFAR10', 'CIFAR100', 'CIFAR100H']:
        ...
        crowd_root_map = {'Benthic': './Benthic', 'Plankton': './Plankton', 'Treeversity': './Treeversity#6'}
        ...
        base_train_ds = Crowdsource(lpi_args, splits=['fold1','fold4','fold5'], transform=None)
        test_ds = Crowdsource(lpi_args, splits=['fold2'], transform=test_t)
